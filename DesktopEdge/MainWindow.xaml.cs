...
        }
			...
			this.Dispatcher.Invoke(() => {
				...
				if (ids.Length > 0 && serviceClient.Connected) {
					...
					foreach (var id in ids) {
						...
						if (id.IsEnabled) idItem.ToggleStatus.Content = "启用";
						else idItem.ToggleStatus.Content = "禁用";
		...
		async private void Disconnect(object sender, RoutedEventArgs e) {
			try {
				...
				ShowError("停止服务失败", "停止服务时发生错误，服务是否正常运行中");
			}
			HideLoad();
		}
